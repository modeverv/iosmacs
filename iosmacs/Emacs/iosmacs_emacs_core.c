...
static char *copy_runtime_eval_form(void) {
    const char *form =
        ...
        "(defun iosmacs-apply-gc-threshold-ab () "
        "(let ((mb (string-to-number (or (getenv \"IOSMACS_GC_THRESHOLD_MB\") \"0\")))) "
        "(when (> mb 0) "
        "(setq gc-cons-threshold (* mb 1024 1024)) "
        "(princ (format \"iosmacs-gc-threshold-mb:%s\\n\" mb) "
        "'external-debugging-output)))) "
        "(iosmacs-apply-gc-threshold-ab) "
        "(add-hook 'emacs-startup-hook #'iosmacs-apply-gc-threshold-ab) "
        ...
        "(condition-case err "
        "(let ((file (locate-library name))) "
        "(when file (load file nil t))) "
        ...
        "(defun iosmacs-register-bundled-commands () "
        "(autoload 'dired \"dired\" nil t) "
        "(autoload 'tetris \"tetris\" nil t)) "
        "(iosmacs-register-bundled-commands) "
        "(add-hook 'emacs-startup-hook #'iosmacs-register-bundled-commands) "
        "(defun iosmacs-force-xterm-input-decode () "
        "(condition-case err "
        "(let ((xterm-extra-capabilities nil) "
        "(skip-init (getenv \"IOSMACS_SKIP_XTERM_INIT\")) "
        "(light-init (getenv \"IOSMACS_LIGHT_XTERM_INIT\"))) "
        "(require 'term/xterm) "
        "(when (and (not skip-init) (not light-init) "
        "(fboundp 'terminal-init-xterm)) "
        "(terminal-init-xterm)) "
        "(when light-init "
        "(when (fboundp 'tty-set-up-initial-frame-faces) "
        "(tty-set-up-initial-frame-faces)) "
        "(when (boundp 'xterm-standard-colors) "
        "(dolist (color xterm-standard-colors) "
        "(when (fboundp 'tty-color-define) "
        "(ignore-errors (apply #'tty-color-define color))))) "
        "(princ \"iosmacs-light-xterm-init-ready\\n\" "
        "'external-debugging-output)) "
        "(when skip-init "
        "(princ \"iosmacs-xterm-init-skipped\\n\" "
        "'external-debugging-output)) "
        "(when (and (boundp 'xterm-paste-ending-sequence) "
        "(fboundp 'xterm--pasted-text)) "
        "(defun iosmacs-fast-xterm-pasted-text () "
        ...
        "(let ((event (read-event nil nil most-positive-fixnum))) "
        "(when (eql event ?\\r) (setf event ?\\n)) "
        "(insert event))) "
        "(let ((last-coding-system-used)) "
        "(decode-coding-region (point-min) (point) "
        "(keyboard-coding-system) t))))) "
        "(fset 'xterm--pasted-text #'iosmacs-fast-xterm-pasted-text)) "
        "(when (fboundp 'xterm-translate-bracketed-paste) "
        "(define-key input-decode-map \"\\e[200~\" #'xterm-translate-bracketed-paste) "
        "(define-key function-key-map \"\\e[200~\" #'xterm-translate-bracketed-paste) "
        "(define-key local-function-key-map \"\\e[200~\" #'xterm-translate-bracketed-paste)) "
        "(princ \"iosmacs-xterm-input-decode-ready\\n\" 'external-debugging-output)) "
        "(error "
        "(princ (format \"iosmacs-xterm-init-error:%S\\n\" err) "
        "'external-debugging-output)))) "
        "(iosmacs-force-xterm-input-decode) "
        "(add-hook 'emacs-startup-hook #'iosmacs-force-xterm-input-decode) "
